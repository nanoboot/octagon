 ...
 * @since 0.1.0
 */
public class RepositoryRegistryImplFactory {
    @Setter
    private EntityRegistryImpl entityRegistry;
    public RepositoryRegistryImpl create() {

        RepositoryRegistryImpl repositoryRegistry = new RepositoryRegistryImpl();
        List<Repository> allRepos = new ArrayList<>();
        for(EntityRegistryEntry e:entityRegistry.list()) {
            Repository repository = e.getRepository();
            ((RepositoryImpl) repository).setRepositoryRegistry(repositoryRegistry);
            allRepos.add(repository);
        }
        repositoryRegistry.loadList(allRepos);
