 ...
 * @since 0.1.0
 */
public class TypeHandlersListFactory {
    @Setter
    private PluginLoader pluginLoader;

    public List<TypeHandler<?>> create() {
        List<TypeHandler<?>> result = new ArrayList<>();
        for (PluginStub pluginStub : pluginLoader.getPluginStubs()) {

            for (TypeHandler<?> typeHandler : pluginStub.getTypeHandlers()) {
                result.add(typeHandler);
            }
        }
        for(TypeHandler<?> t:result) {
            System.out.println("type handler:" + t.getClass().getName());
        }
        return result;
    }
}
